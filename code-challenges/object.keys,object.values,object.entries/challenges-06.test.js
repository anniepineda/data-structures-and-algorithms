...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(name => houses.push(name.house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let children = false;
  arr.forEach(homeCharacter => {
    if (homeCharacter.name === character) {
      let value = Object.values(homeCharacter);
      if (value[2].length > 0) {
        children = true;
      }
    }
...
const hasChildrenEntries = (arr, character) => {
  let children = false;
  arr.forEach(homeCharacter => {
    let dataEntry = Object.entries(homeCharacter)
    if (dataEntry[0][1] === character) {
      if (dataEntry[2][1].length > 0) {
        children = true;
      }
    }
  });
...
/* Hollie helped me with this one ------------------------------------------------------------------------------------------------
